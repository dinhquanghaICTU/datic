...
int wifi_if_disable(void);  /* Completely disable WiFi STA to release radio */
int wifi_if_enable(void);   /* Re-enable WiFi STA after disable */
bool wifi_if_is_connected(void);
bool wifi_if_is_mgmr_ready(void);

/* Allow app layer to register WiFi status callbacks */
